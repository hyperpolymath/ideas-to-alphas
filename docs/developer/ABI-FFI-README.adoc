...
# KATAGORIA ABI/FFI Documentation
...
│       └── include/
│           └── katagoria.h   # C header (optional, can be generated)
│
├── generated/                  # Auto-generated files
│   └── abi/
│       └── katagoria.h       # Generated from Idris2 ABI
│
...
```bash
cd src/abi
idris2 --cg c-header Types.idr -o ../../generated/abi/katagoria.h
```
...
int main() {
    void* handle = katagoria_init();
    if (!handle) return 1;

    int result = katagoria_process(handle, 42);
    if (result != 0) {
        const char* err = katagoria_last_error();
        fprintf(stderr, "Error: %s\n", err);
    }

    katagoria_free(handle);
    return 0;
}
```
...
Compile with:
```bash
gcc -o example example.c -lkatagoria -L./zig-out/lib
```

### From Idris2

```idris
import KATAGORIA.ABI.Foreign
...
#[link(name = "katagoria")]
extern "C" {
    fn katagoria_init() -> *mut std::ffi::c_void;
    fn katagoria_free(handle: *mut std::ffi::c_void);
    fn katagoria_process(handle: *mut std::ffi::c_void, input: u32) -> i32;
}

fn main() {
    unsafe {
        let handle = katagoria_init();
        assert!(!handle.is_null());

        let result = katagoria_process(handle, 42);
        assert_eq!(result, 0);
...
```julia
const libkatagoria = "libkatagoria"

function init()
    handle = ccall((:katagoria_init, libkatagoria), Ptr{Cvoid}, ())
    handle == C_NULL && error("Failed to initialize")
    handle
end

function process(handle, input)
    result = ccall((:katagoria_process, libkatagoria), Cint, (Ptr{Cvoid}, UInt32), handle, input)
    result
end

function cleanup(handle)
    ccall((:katagoria_free, libkatagoria), Cvoid, (Ptr{Cvoid},), handle)
end
...
2. **Generate C header**
   ```bash
   idris2 --cg c-header src/abi/Types.idr -o generated/abi/katagoria.h
   ```
