...
*This repository is not finished being set up.* 25 substitution token(s)
across 21 file(s) still have no value.
...
* `+{{DESCRIPTION}}+` — One-line description used in
.github/settings.yml. HIGH PRIORITY: settings.yml is applied by a GitHub
App, so an unfilled token here can be written into forge metadata
verbatim.
...
One-line description used in .github/settings.yml. HIGH PRIORITY:
settings.yml is applied by a GitHub App, so an unfilled token here can
be written into forge metadata verbatim.
